...
# ── Configuration ──────────────────────────────────────────────────────────────
USERNAME = (
    os.environ.get("GITHUB_USERNAME", "").strip()
...
if not GITHUB_TOKEN:
    try:
        ...
        GITHUB_TOKEN = ""
        print("WARNING: No GitHub token was found. Continuing in unauthenticated mode with lower rate limits.")
...
def main():
    ...
    for idx, (ps, pe) in enumerate(periods, 1):
        ...
        if today < pe:
            ...
            report_infos.append(((ps, pe), filepath, (0, 0, 0)))
            print()
            continue
